...
#     def __init__(self, loc, operation='activation', input_name='x'):
    def __init__(self, loc, operation='activation_149', input_name='x'):
        """
        ...
        self.sess = tfc.Session(graph=self.graph)
        with self.graph.as_default():
            saver = tfc.train.import_meta_graph(loc + '.meta', clear_devices=True)
            saver.restore(self.sess, loc)
            print("recovring the model")
#           self.op = self.graph.get_operation_by_name(operation).outputs[0]
            self.op = self.graph.get_operation_by_name(operation).outputs[0]
            print("error is ", self.op)
